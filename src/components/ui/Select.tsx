...
export function Select({
  ...
  const [listboxMaxHeight, setListboxMaxHeight] = useState(256);
  ...
  const open = (preferredIndex = selectedIndex) => {
    ...
    if (triggerRect) {
      ...
      const shouldOpenUpward = availableBelow < 192 && availableAbove > availableBelow;
      const availableSpace = shouldOpenUpward ? availableAbove : availableBelow;

      setOpensUpward(shouldOpenUpward);
      setListboxMaxHeight(Math.min(256, Math.max(0, availableSpace - 12)));
    }
    ...
    >
      ...
      {isOpen && (
        <div
          ...
          className={`absolute left-0 right-0 z-50 overflow-y-auto rounded-2xl border border-white/10 bg-[#111] p-1.5 shadow-2xl shadow-black/60 ${opensUpward ? "bottom-full mb-2" : "top-full mt-2"}`}
        >
