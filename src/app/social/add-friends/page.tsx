...
import { ChevronLeft, Flame, Star, CheckCircle2, UserPlus, Search } from "lucide-react";
...
export default function AddFriendsPage() {
  ...
  // Filter out people who are already friends or pending
  const discoverable = friends.filter(f => f.status === 'none');
    ...
    <main className="flex min-h-screen flex-col bg-[#050505] relative overflow-x-hidden pb-24">
      ...
      <header className="fixed top-0 w-full z-50 flex items-center justify-between p-4 pt-[calc(var(--notch-top)+1rem)]">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-black/40 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white">
      ...
      <section className="w-full px-6 mt-8 z-10 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-black text-white">Recommended Athletes</h2>
          <button className="text-xs font-bold text-purple-400 bg-purple-400/10 px-3 py-1 rounded-full">View all &rarr;</button>
        </div>

        <div className="flex overflow-x-auto no-scrollbar gap-4 pb-8 pr-6 -mr-6 pl-1">
          {discoverable.map((user, idx) => (
            <div 
              key={user.id}
            ...
            >
              ...
              <h3 className="text-white font-bold z-10">{user.name}</h3>
              <div className="flex items-center gap-1 mt-1 z-10">
                <Flame className={`w-3 h-3 ${idx % 2 === 0 ? 'text-accent-green' : 'text-purple-400'}`} />
                <span className="text-[10px] text-text-muted font-bold">{user.activeRoutine}</span>
              </div>
              ...
              <div className="relative mt-4 mb-6 z-10">
                ...
                <img src={user.avatar} className="w-16 h-16 rounded-full border-2 border-black relative z-10" />
              </div>
